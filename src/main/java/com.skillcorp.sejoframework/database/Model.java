...
public abstract class Model {
	...
	public void save(boolean loadLast)
	{
		this.save();
		this.last();
	}
	...
	{
		return this.db.table(this.table, null);
	}
		...
		for( Field field : fields ){
			...
			try {
				...
				if(addField)
				{
					...
					if(field.getName().equals("timestamps")){
						if((boolean)field.get(obj)==true){
							Date now = new Date();
	...
	{
		setProperties(this,createObjectProperties(data));
	}
	...
	private <T> void setProperties(Object obj, Map<String, String> data)
	{
		...
		for( Field field : fields ){
			...
			try {
				...
				if(field.getName().equals("timestamps"))
				{
					boolean hasTimes = (boolean) field.get(obj);
					if(hasTimes)
					{
						try {
							SimpleDateFormat dt = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
							this.created_at = dt.parse(data.get("created_at"));
							this.updated_at = dt.parse(data.get("updated_at"));
						} catch (ParseException e) {
